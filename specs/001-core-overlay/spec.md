...
As a driver, I want one compact ribbon that shows progress, safety, and key car controls so I can make decisions within a few seconds while staying focused on the track.

**Why this priority**: Consolidated race status reduces cognitive load and helps prevent avoidable incidents.

**Independent Test**: Can be tested by feeding telemetry snapshots for laps, timed sessions, incidents, brake bias, and traction control, then verifying the displayed values and update responsiveness.

**Acceptance Scenarios**:

1. **Given** an active race session, **When** lap/progress/safety telemetry changes, **Then** the ribbon shows updated current-versus-total progress and incidents-versus-limit values.
2. **Given** a timed session without fixed total laps, **When** remaining session time changes, **Then** the ribbon shows estimated lap progress derived from available lap-time context.
3. **Given** driver control-dial adjustments, **When** brake bias or traction-control telemetry changes, **Then** the ribbon reflects updated values on the next render cycle.
...
- Timed session has no recent valid lap-time sample for progress estimation.
...
- **FR-003**: System MUST render a top-position rev strip using 15 to 20 visual segments.
...
- **FR-008**: System MUST display progress as current lap versus total/estimated laps.
- **FR-009**: System MUST display incidents as current count versus allowed limit.
...
- **Race Ribbon Snapshot**: Current display values for lap progress, incidents, brake bias, and traction control.
- **Session Progress Context**: Telemetry-derived values used to compute total/estimated lap progress.
...
- **SC-002**: In timed and lap-limited sessions, displayed lap progress matches expected value calculation in at least 99% of sampled frames.
